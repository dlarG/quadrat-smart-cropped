          ...
          {results && (
            <div className="space-y-6">
              ...
              {results.rectified_quadrats.length > 0 && (
                <div className="bg-white rounded-lg shadow-lg p-6">
                  <h3 className="text-lg font-semibold mb-4">
                    Rectified Quadrats & Coral Analysis
                  </h3>
                    ...
                    {results.rectified_quadrats.map((quadrat) => (
                      ...
                      >
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                          ...
                          {/* Coral Segmentation Overlay */}
                          {quadrat.coral_overlay_base64 && (
                            <div>
                              <h5 className="font-medium mb-2 text-gray-700">
                                Coral Overlay
                              </h5>
                              <img
                                src={`data:image/png;base64,${quadrat.coral_overlay_base64}`}
                                alt={`Coral Overlay ${quadrat.id}`}
                                className="w-full h-48 object-cover rounded-lg border"
                              />
                              <p className="text-sm text-gray-600 mt-1">
                                Total Coral:{" "}
                                {
                                  quadrat.coral_segmentation
                                    ?.total_coral_coverage
                                }
                                %
                              </p>
                            </div>
                          ...
                          {/* Coral Mask */}
                          {quadrat.coral_mask_base64 && (
                            <div>
                              <h5 className="font-medium mb-2 text-gray-700">
                                Segmentation Mask
                              </h5>
                              <img
                                src={`data:image/png;base64,${quadrat.coral_mask_base64}`}
                                alt={`Coral Mask ${quadrat.id}`}
                                className="w-full h-48 object-cover rounded-lg border"
                              />
                              <p className="text-sm text-gray-600 mt-1">
                                Color-coded coral types
                              </p>
                            </div>
                          )}
                        </div>

                        {/* Coral Coverage Statistics */}
                        {quadrat.coral_segmentation &&
                          quadrat.coral_segmentation.coverage_data.length >
                            0 && (
                            <div className="bg-white rounded-lg p-4 border">
                              <h5 className="font-semibold mb-3 text-gray-800">
                                Coral Coverage Analysis
                              </h5>
                              ...
                              {/* Individual Coral Types */}
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                {quadrat.coral_segmentation.coverage_data.map(
                                  (coral, index) => (
                                    <div
                                      key={index}
                                      className="flex items-center justify-between p-2 bg-gray-50 rounded border-l-4"
                                      style={{ borderLeftColor: coral.color }}
                                    >
                                      <div>
                                        ...
                                        <div className="text-xs text-gray-600">
                                          {coral.pixel_count.toLocaleString()}{" "}
                                          pixels
                                        </div>
                                      </div>
                            ...
                            <div className="flex items-start gap-3">
                              ...
                              <div>
                                <h6 className="font-medium text-yellow-800 mb-1">
                                  Coral Analysis Unavailable
                                </h6>
                                <p className="text-sm text-yellow-700">
                                  The coral segmentation model could not be
                                  loaded or no coral was detected in this
                                  quadrat. The quadrat has been successfully
                                  rectified and is ready for manual analysis.
                                </p>
                              </div>
                            </div>
                          </div>
